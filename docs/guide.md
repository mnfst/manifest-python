...
- Standard httpx and httpx2 transports, requests adapters and aiohttp 3.13+ sessions are instrumented process-wide, including existing clients. aiohttp is covered through its client middleware chain; the SDK's middleware runs innermost, so your own middlewares and `raise_for_status` see the result of the heal. Custom transports, `urllib`, aiohttp before 3.13, websocket handshakes, browsers and other languages are not covered.
