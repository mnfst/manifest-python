...
from mnfst import HealEvent, init
...
def test_installs_outbound():
    init(key="mnfx_k", url="http://first.test")
...
def test_takes_no_positional_app():
    ...
    with pytest.raises(TypeError):
        init(object())  # type: ignore[call-arg]
...
def test_second_install_with_different_config_warns():
    ...
    with warnings.catch_warnings(record=True) as caught:
        warnings.simplefilter("always")
        init(key="mnfx_k", url="http://second.test")
...
def test_exports():
    ...
    assert mnfst_pkg.__all__ == ["init", "flush", "HealEvent", "VERSION"]
